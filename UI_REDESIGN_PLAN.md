...
| 3 其他畫面 | ✅ 大部分 | v1.8.8：首頁進行中／最近成果、所有作品↔素材庫、長片組合（說明鈕／加入前檢查／保留排列／復原／LruCache）、教學範本卡、設定五組、Grok 狀態列＋貼上。**未做**：3.4 LessonCard 改 ImagineCard＋MediaActionBar（僅改文案）、3.5 常數抽到 Constants.kt |
| 5 日常效率 | ✅ 全部 | 匯入回饋含失敗數、往前移／往後移、提示詞檢查改名、佔位符提示 |
| 6 設定儲存瀏覽 | ⚠️ 部分 | ✅ 6.1 測試連線、6.2 費用預估、6.4 分享附提示詞、6.6 播放器靜音／循環、6.7 通知權限＋直達作品、6.5 搜尋無結果文案。**未做**：6.3 空間與快取／僅 Wi-Fi、6.5 失敗記錄與排序、6.4 多張分享／可讀檔名 |
| 4 視覺收斂 | ⚠️ 部分 | 死參數 showBalanceBar／死路由 VIDEO_GENERATING 已清；硬編碼 sp/dp 收斂與對比度檢查未做 |
| 7 可讀性無障礙 | ⚠️ 部分 | 圖示 contentDescription 預設名稱、縮圖 ✕／移除鈕觸控區放大；11sp 標籤全面調整與空白狀態全套未做 |

實機驗收待使用者：主流程（生圖 → 圖片動起來 → 延長並組合）、S22 Ultra 放大字型、鍵盤開啟時主按鈕、OpenRouter 路徑。
