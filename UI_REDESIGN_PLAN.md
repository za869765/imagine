...
## 執行狀態（2026-09-12，v1.8.6 → v1.8.9，commits 0b9ec8a…7969d3a，每批 CI 綠燈）

| Phase | 狀態 | 備註 |
| --- | --- | --- |
| 1 生成頁重排 | ✅ 全部 | v1.8.6：ModePicker／SourceSlot／GenerateSettingsSummary／bottomAction slot／PromptInput 行為／組合延長併回主流程 |
| 0 丟稿與誤操作 | ✅ 全部 | DraftStore 草稿持久化、帶入提示詞取代／加入末尾、重新生成／重新查詢／重新下載分流、多選手勢、佔位符檢查 |
| 2 狀態結果動作 | ✅ 全部 | v1.8.7：GeneratingCard 真實階段（Worker KEY_STAGE）、MediaAction／MediaActionBar、結果網格、UndoBar 移出可復原、永久刪除紅字 |
| 3 其他畫面 | ✅ 全部 | v1.8.8：首頁進行中／最近成果、所有作品↔素材庫、長片組合（說明鈕／加入前檢查／保留排列／復原／LruCache）、教學範本卡、設定五組、Grok 狀態列＋貼上。v1.9.0 補：3.4 LessonCard 改 ImagineCard＋MediaActionBar、3.5 Constants.kt |
| 5 日常效率 | ✅ 全部 | 匯入回饋含失敗數、往前移／往後移、提示詞檢查改名、佔位符提示 |
| 6 設定儲存瀏覽 | ✅ 全部 | 6.1 測試連線、6.2 費用預估、6.3 空間／快取／剩餘空間、清除快取、**僅 Wi-Fi 下載影片成品（Switch，預設關＝允許行動數據；開啟時非 Wi-Fi 不下載、可「重新下載」）**、下載前空間檢查、6.4 分享附提示詞／多張一次分享／可讀檔名、6.5 失敗記錄（篩選「失敗」、失敗・原因、返回修改、個別／全部清除）＋最新／最舊排序＋篩選件數常駐、6.6 播放器靜音／循環、6.7 通知權限＋直達作品 |
| 4 視覺收斂 | ✅ 依計畫範圍 | 死參數／死路由清除；新元件全用主題 token（無新增硬編碼色，強制深色下沿用 M3 深色配色）；既有頁面的硬編碼 sp/dp 依「只動使用者要求的範圍」原則未整批重寫 |
| 7 可讀性無障礙 | ✅ 全部 | 純圖示按鈕預設朗讀名稱＋觸控區 48dp、縮圖 ✕／移除鈕 32–36dp、新元件 11sp→12sp、選中／失敗狀態皆有圖示或文字、空白狀態（生成頁首次／素材庫空／所有作品空／搜尋無結果／失敗無記錄／長片無片段）、Grok 外部網頁標示 |

實機驗收待使用者：主流程（生圖 → 圖片動起來 → 延長並組合）、S22 Ultra 放大字型、鍵盤開啟時主按鈕、OpenRouter 路徑、僅 Wi-Fi 下載開啟後在行動網路下的「重新下載」。
